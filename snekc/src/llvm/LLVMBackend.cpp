...
static LLVMTypeRef GenFunctionType(LLVMBackend* llb, SkModule* module, TypeID type, AST::FunctionType* ast)
{
	...
	LLVMTypeRef functionType = FunctionType(llb, module, returnType, paramTypes, numParams, type->functionType.varArgs);
...
static LLVMTypeRef GenArrayType(LLVMBackend* llb, SkModule* module, TypeID type, AST::ArrayType* ast)
{
	...
	*/
	...
	{
		return GetSliceType(llb, elementType);
	}
}

LLVMTypeRef GenTypeID(LLVMBackend* llb, SkModule* module, TypeID type, AST::Type* ast)
{
...
static LLVMValueRef GetVariableValue(LLVMBackend* llb, SkModule* module, Variable* variable, AST::File* currentFile)
{
	...
	*/
	//{
	if (currentFile == variable->declaration->file || variable->isConstant)
	{
		value = (LLVMValueRef)variable->allocHandle;
	}
	else
	{
		// Import global
		auto it = module->globalValues.find((AST::GlobalVariable*)variable->declaration);
		if (it != module->globalValues.end())
			value = it->second;
		else
		{
			value = GenGlobalHeader(llb, module, (AST::GlobalVariable*)variable->declaration);
		}
	}
	//}
	return value;
...
static LLVMValueRef GenInitializerList(LLVMBackend* llb, SkModule* module, AST::InitializerList* expression)
{
	/*
	...
	if (false)
	{
		...
		if (expression->initializerType->typeKind == AST::TypeKind::Struct)
		{
			LLVMTypeRef llvmType = GenType(llb, module, expression->initializerType);
			...
			for (int i = 0; i < numFields; i++)
			{
				LLVMTypeRef fieldType = GenTypeID(llb, module, expression->initializerType->typeID->structType.fieldTypes[i]);
				if (i < expression->values.size)
				{
					...
					if (expression->values[i]->isConstant())
						field = ConstCastValue(llb, module, field, fieldType, expression->values[i]->valueType, expression->initializerType->typeID->structType.fieldTypes[i]);
					else
						field = CastValue(llb, module, field, fieldType, expression->values[i]->valueType, expression->initializerType->typeID->structType.fieldTypes[i]);
					values[i] = field;
		...
		else if (expression->initializerType->typeKind == AST::TypeKind::Array)
		{
			int arrayLength = expression->initializerType->typeID->arrayType.length;

			LLVMTypeRef elementType = GenTypeID(llb, module, expression->initializerType->typeID->arrayType.elementType);
			LLVMTypeRef arrayType = LLVMArrayType(elementType, arrayLength);
			LLVMValueRef alloc = AllocateLocalVariable(llb, module, arrayType, "");

			for (int i = 0; i < arrayLength; i++)
			{
				LLVMValueRef index = LLVMConstInt(LLVMInt32TypeInContext(llb->llvmContext), i, false);
				LLVMValueRef valueAlloc = LLVMBuildGEP(module->builder, alloc, &index, 1, "");

				if (i < expression->values.size)
				{
					LLVMValueRef value = GetRValue(llb, module, GenExpression(llb, module, expression->values[i]), expression->values[i]->lvalue);
					if (expression->values[i]->isConstant())
						value = ConstCastValue(llb, module, value, elementType, expression->values[i]->valueType, expression->initializerType->typeID->structType.fieldTypes[i]);
					else
						value = CastValue(llb, module, value, elementType, expression->values[i]->valueType, expression->initializerType->typeID->structType.fieldTypes[i]);

					LLVMBuildStore(module->builder, value, valueAlloc);
				}
				else
				{
					LLVMBuildStore(module->builder, LLVMConstNull(elementType), valueAlloc);
				}
			}
	...
	*/
	...
	if (expression->initializerType->typeKind == AST::TypeKind::Struct)
	{
		LLVMTypeRef llvmType = GenTypeID(llb, module, expression->initializerType);
		LLVMValueRef alloc = LLVMBuildAlloca(module->builder, llvmType, "");
		for (int i = 0; i < expression->values.size; i++)
		{
			LLVMValueRef field = GenExpression(llb, module, expression->values[i]);
			LLVMTypeRef fieldType = GenTypeID(llb, module, expression->initializerType->structType.fieldTypes[i]);
			field = CastValue(llb, module, field, fieldType, expression->values[i]->valueType, expression->initializerType->structType.fieldTypes[i], expression->values[i]->lvalue);
			LLVMValueRef fieldAlloc = LLVMBuildStructGEP(module->builder, alloc, i, "");
			LLVMBuildStore(module->builder, field, fieldAlloc);
		}
		return LLVMBuildLoad(module->builder, alloc, "");
	...
	{
		int arrayLength = expression->initializerType->arrayType.length;

		LLVMTypeRef elementType = GenTypeID(llb, module, expression->initializerType->arrayType.elementType);
		LLVMTypeRef arrayType = GetArrayType(llb, elementType, arrayLength);
		LLVMValueRef alloc = AllocateLocalVariable(llb, module, arrayType, "");

		for (int i = 0; i < arrayLength; i++)
		{
			LLVMValueRef valueAlloc = GetArrayElementAlloc(llb, module, alloc, LLVMConstInt(LLVMInt32TypeInContext(llb->llvmContext), i, false));
...
static LLVMValueRef GenIdentifier(LLVMBackend* llb, SkModule* module, AST::Identifier* expression)
{
	...
	{
		return GetVariableValue(llb, module, expression->variable, expression->file);
	}
	else if (expression->functions.size > 0)
	{
		if (expression->functions[0]->file == expression->file)
			return module->functionValues[expression->functions[0]];
		else
			return GenFunctionHeader(llb, module, expression->functions[0]);
...
static LLVMValueRef GenFunctionCall(LLVMBackend* llb, SkModule* module, AST::FunctionCall* expression)
{
	...
	for (int i = 0; i < max(numArgs, numParams); i++)
	{
		...
		if (i < paramCount)
		{
			if (i < numArgs)
			{
				arg = GenExpression(llb, module, expression->arguments[i]);

				LLVMTypeRef paramType = GenTypeID(llb, module, functionType->functionType.paramTypes[paramOffset + i]);
				arg = ConvertValue(llb, module, arg, paramType, expression->arguments[i]->valueType, functionType->functionType.paramTypes[paramOffset + i], expression->arguments[i]->lvalue, expression->arguments[i]->isConstant());
			...
			else
			{
				...
				SnekAssert(expression->function);
				arg = GenExpression(llb, module, expression->function->paramValues[i]);
			}
		}
		else
		{
			...
			if (expression->arguments[i]->valueType->typeKind == AST::TypeKind::FloatingPoint)
				arg = CastValue(llb, module, arg, LLVMDoubleTypeInContext(llb->llvmContext), expression->arguments[i]->valueType, GetFloatingPointType(FloatingPointPrecision::Double), false);
		}

		arguments.add(arg);
	}

	LLVMValueRef result = CallFunction(llb, module, functionType, callee, arguments.buffer, arguments.size);
...
static LLVMValueRef GenSubscriptOperator(LLVMBackend* llb, SkModule* module, AST::SubscriptOperator* expression)
{
		...
		expression->operand->valueType->typeKind == AST::TypeKind::Pointer && expression->operand->valueType->pointerType.elementType->typeKind == AST::TypeKind::Array)
	{
		...
		TypeID type = expression->operand->valueType;

		if (expression->lvalue && type->typeKind == AST::TypeKind::Pointer)
		{
			operand = LLVMBuildLoad(module->builder, operand, "");
			type = type->pointerType.elementType;
		}
		...
		if (type->arrayType.length != -1)
			return GetArrayElementAlloc(llb, module, operand, index);
		else
			return GetSliceElementAlloc(llb, module, operand, index);
	}
	else if (expression->operand->valueType->typeKind == AST::TypeKind::String)
	{
		...
		LLVMValueRef index = GenExpression(llb, module, expression->arguments[0]);
		index = GetRValue(llb, module, index, expression->arguments[0]->lvalue);
		LLVMValueRef element = GetStringElementAlloc(llb, module, operand, index);
...
{
	if (expression->namespacedVariable)
	{
		return GetVariableValue(llb, module, expression->namespacedVariable, expression->file);
	}
	else
	{
		LLVMValueRef operand = GenExpression(llb, module, expression->operand);
		...
		else if (operandType->typeKind == AST::TypeKind::Array)
		{
			...
			if (operandType->arrayType.length != -1)
			{
				if (expression->arrayField == 0)
					return GetArraySize(llb, module, operand);
			...
			{
				if (expression->arrayField == 0)
					return GetSliceSize(llb, module, operand);
				else if (expression->arrayField == 1)
					return GetSliceBuffer(llb, module, operand);
				else
				{
					SnekAssert(false);
					return NULL;
...
static LLVMValueRef GenCast(LLVMBackend* llb, SkModule* module, AST::Typecast* expression)
{
	...
	return CastValue(llb, module, value, type, expression->value->valueType, expression->dstType->typeID, expression->value->lvalue);
}
...
static LLVMValueRef GenMalloc(LLVMBackend* llb, SkModule* module, AST::Malloc* expression)
{
	...
	if (expression->dstType->typeKind == AST::TypeKind::Array)
	{
		AST::ArrayType* arrayType = (AST::ArrayType*)expression->dstType;

		SnekAssert(arrayType->length);

		LLVMTypeRef elementType = GenTypeID(llb, module, expression->dstType->typeID->arrayType.elementType);

		LLVMValueRef length = GenExpression(llb, module, arrayType->length);
		length = CastValue(llb, module, length, LLVMInt32TypeInContext(llb->llvmContext), arrayType->length->valueType, GetIntegerType(32, true), arrayType->length->lvalue);
		if (arrayType->length->isConstant())
			alloc = CreateArrayOfSize(llb, module, elementType, length, expression->malloc);
		else
		{
			SnekAssert(expression->malloc);
			alloc = CreateSliceOfSize(llb, module, elementType, length);
		}
	}
	else if (expression->dstType->typeKind == AST::TypeKind::String)
	{
		SnekAssert(!expression->count);

		AST::StringType* stringType = (AST::StringType*)expression->dstType;

		SnekAssert(stringType->length);

		LLVMValueRef length = GenExpression(llb, module, stringType->length);
		length = CastValue(llb, module, length, LLVMInt32TypeInContext(llb->llvmContext), stringType->length->valueType, GetIntegerType(32, true), stringType->length->lvalue);
		alloc = CreateStringOfSize(llb, module, length, expression->malloc);
	}
	else
	{
		if (expression->count)
		{
			...
			if (expression->malloc)
				alloc = LLVM_CALL(LLVMBuildArrayMalloc, module->builder, llvmType, count, "");
			else
				alloc = LLVM_CALL(LLVMBuildArrayAlloca, module->builder, llvmType, count, "");
		}
		else
		{
			if (expression->malloc)
				...
				alloc = LLVM_CALL(LLVMBuildAlloca, module->builder, llvmType, "");
		}
	}
		...
		if (AST::Function* constructor = declaration->constructor)
		{
			...
			for (int i = 1; i < numArgs; i++)
			{
				...
				if (i < numParams)
				{
					...
					arg = ConvertValue(llb, module, arg, paramType, argExpr->valueType, paramTypeID, argExpr->lvalue, LLVMIsConstant(arg));
				}
				else
				{
					arg = GetRValue(llb, module, arg, argExpr->lvalue);
					if (argExpr->valueType->typeKind == AST::TypeKind::FloatingPoint)
						arg = CastValue(llb, module, arg, LLVMDoubleTypeInContext(llb->llvmContext), argExpr->valueType, GetFloatingPointType(FloatingPointPrecision::Double), false);
				}
				arguments[i] = arg;
			}
...
static LLVMValueRef GenExpression(LLVMBackend* llb, SkModule* module, AST::Expression* expression)
{
	...
	case AST::ExpressionType::InitializerList:
		return GenInitializerList(llb, module, (AST::InitializerList*)expression);
...
static void GenVarDeclStatement(LLVMBackend* llb, SkModule* module, AST::VariableDeclaration* statement)
{
	...
	for (int i = 0; i < statement->declarators.size; i++)
	{
		...
		if (declarator->value)
		{
			...
			if (declarator->value->valueType->typeKind == AST::TypeKind::Pointer && CompareTypes(declarator->value->valueType->pointerType.elementType, statement->varType->typeID))
			{
				alloc = value;
				LLVMSetValueName(alloc, name);
			}
			else if (declarator->value->valueType->typeKind == AST::TypeKind::Array && declarator->value->type == AST::ExpressionType::InitializerList)
			{
				alloc = value;
				LLVMSetValueName(alloc, name);
			}
			...
			{
				value = ConvertValue(llb, module, value, type, declarator->value->valueType, statement->varType->typeID, declarator->value->lvalue, LLVMIsConstant(value));

				alloc = AllocateLocalVariable(llb, module, type, name);
...
static void GenForLoop(LLVMBackend* llb, SkModule* module, AST::ForLoop* statement)
{
	...
	LLVMValueRef it = AllocateLocalVariable(llb, module, LLVMInt32TypeInContext(llb->llvmContext), statement->iteratorName->name);
	if (module->hasDebugInfo)
	{
		DebugInfoDeclareVariable(llb, module, it, GetIntegerType(32, true), statement->iteratorName->name, statement->location);
	}
	...
	//end = GetRValue(llb, module, end, );
	//SnekAssert(LLVMGetTypeKind(LLVMTypeOf(end)) == LLVMIntegerTypeKind);
	end = CastValue(llb, module, end, LLVMInt32TypeInContext(llb->llvmContext), statement->endValue->valueType, GetIntegerType(32, false), statement->endValue->lvalue);

	if (statement->deltaValue) {
		delta = GenExpression(llb, module, statement->deltaValue);
		//delta = GetRValue(llb, module, delta, statement->deltaValue->lvalue);
		delta = ConvertValue(llb, module, delta, LLVMInt32TypeInContext(llb->llvmContext), statement->deltaValue->valueType, GetIntegerType(32, true), statement->deltaValue->lvalue, statement->deltaValue->isConstant());
	...
	LLVMIntPredicate predicate = statement->includeEndValue ? LLVMIntSLE : LLVMIntSLT;
	LLVMValueRef condition = LLVM_CALL(LLVMBuildICmp, module->builder, predicate, itValue, end, "");
	LLVM_CALL(LLVMBuildCondBr, module->builder, condition, loopBlock, mergeBlock);
...
static void GenReturn(LLVMBackend* llb, SkModule* module, AST::Return* statement)
{
	if (statement->value)
	{
		...
		//value = GetRValue(llb, module, value, );
		value = CastValue(llb, module, value, returnType, statement->value->valueType, function->returnType->typeID, statement->value->lvalue);
...
static void GenFree(LLVMBackend* llb, SkModule* module, AST::Free* statement)
{
	for (int i = 0; i < statement->values.size; i++)
	{
		...
		else if (statement->values[i]->valueType->typeKind == AST::TypeKind::String)
		{
			SnekAssert(statement->values[i]->lvalue);
			...
			LLVMBuildFree(module->builder, buffer);
...
LLVMValueRef GenFunctionHeader(LLVMBackend* llb, SkModule* module, AST::Function* decl)
{
	...
	else
	{
		...
		LLVMTypeRef returnType = decl->returnType ? GenType(llb, module, decl->returnType) : LLVMVoidTypeInContext(llb->llvmContext);
...
static SkModule** GenModules(LLVMBackend* llb, int numModules, AST::File** asts, bool genDebugInfo)
{
	...
	SnekTrace(llb->context, "Preparing modules");
	...
	SnekTrace(llb->context, "Generating imports");
	...
	SnekTrace(llb->context, "");
	SnekTrace(llb->context, "Generating types:");
	...
	for (int i = 0; i < numModules; i++)
	{
		...
		SnekTrace(llb->context, "%s", module->ast->name);
	...
	SnekTrace(llb->context, "");
	SnekTrace(llb->context, "Generating functions:");
	...
	for (int i = 0; i < numModules; i++)
	{
		...
		for (int j = 0; j < module->ast->functions.size; j++)
		{
			SnekTrace(llb->context, "\t%s", module->ast->functions[j]->name);
